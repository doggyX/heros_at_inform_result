...
- 任务计数器持久化，确保服务重启后编号连续性
- 任务状态管理：
  - `pending`：待验证状态
  - `continue`：第一次验证失败，需要继续验证
  - `success`：验证成功
- 结果变化时的任务处理：
  - 当电气检验结果变化时，检查是否已存在相同的验证任务
  - 如果任务已存在，修改其预期结果，保持原来的状态
  - 如果任务不存在，添加新任务
- 提供任务处理回调接口
- 当continue状态的任务验证成功时，发送复核成功通知
- 支持备注字段传递，在所有通知和记录中显示备注信息

### 4. 配置管理
- 支持特殊路径映射配置
- 支持产品编号到单元格位置的映射配置
- 任务计数器配置文件（task_counter.json），用于持久化任务编号计数
- 环境变量配置钉钉机器人信息
...
`task_counter.json` 文件会自动创建，包含以下内容：
...
- `today_date`：当前日期（格式：YYMMDD）
- `today_task_count`：当天任务计数
...
- `monitor_records.json`：监控记录
- `verify_success_records.json`：验证成功记录
- `task_counter.json`：任务计数器配置文件，持久化存储任务编号计数
...
- v1.3：添加任务状态管理（continue状态）、任务计数器持久化功能、结果变化时的任务处理逻辑和备注字段传递
